...
def DQ_FindSmallestElement(lst):
    ...
    if len(lst) == 3:                   # If Input is 3 sort the input
        ...
        lstOne[0], lstOne[1], lstOne[2] = DQ_FindSmallestElement(lst[:halfList])    # Recursive call whit the first part of the list
        lstTwo[0], lstTwo[1], lstTwo[2] = DQ_FindSmallestElement(lst[halfList:])    # Recursive call whit the secund part of the list
    ...
    awnserLst = []                      # help list to store the correct values
    for mamma in lstOne:                # Goes through  
        for pappa in lstTwo:            # 
            if pappa < mamma:           # 
