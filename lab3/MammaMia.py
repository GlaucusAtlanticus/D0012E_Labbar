...
class BinaryTree:
    ...
    def insertNode(self, value, node = None) -> bool:
        ...
        if node.value < value:                              # IF TRUE: walk right
            if node.right == None:                              # can not walk right
                ...
                node.updateSize()                               # check condition
                return True                                     
            else:
            ...
            if node.left == None:                               # can not walk left
                ...
                node.updateSize()                               # check confition
                return success                                  
    ...
    def insert(self, value):
        ...
        self.CheckConst(value)
        return 

    #TODO add comments
    def CheckConst(self, value, node = None):
        if node == None:
            node = self.root

        parent = node.parent                            # Saves this nodes parent
        max = self.constraint * node.size
        
        if node.value < value and node.right != None:
            if max < node.right.size:
                ...
                self.CheckConst(value, node.right)
        elif node.value < value and node.Left != None:
            if max < node.left.size:
                newSubTree = self.rearrange(node)                   #   Rearrange this sub tree
                if node == self.root:                               # IF old node was tree root
                    self.root = newSubTree                          #   Update tree root
                else:                                               # ELSE
                ...
                self.CheckConst(value, node.left)
        else:
            return False
